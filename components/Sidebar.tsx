...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, language }) => {
  ...
  const menuItems = [
    ...
    { id: 'schemes', label: t.schemesTitle, icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
    ) },
    { id: 'grievances', label: t.grievancesTitle, icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
    ) },
    { id: 'welfare', label: t.welfareTitle, icon: (
  ...
  return (
    <div className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col h-screen fixed left-0 top-0 z-50">
      ...
      <nav className="flex-1 px-4 space-y-1 mt-2 overflow-y-auto custom-scrollbar">
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] px-4 mb-3">Main Menu</p>
        {menuItems.map((item) => (
          <button
            key={item.id}
        ...
        <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700/50">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-200 truncate">Sandeep Kumar</p>
              <p className="text-[10px] font-bold text-indigo-400 uppercase">Citizen User</p>
            </div>
          </div>
          <button 
